  ...
  const handleSendMessage = async (message = inputMessage) => {
    ...
    try {
      ...
      const limitedContext = {
        ...
        chatHistory: Array.isArray(chatMessages) ? chatMessages.slice(-5) : []
      };
      ...
      const botMessage: ChatMessage = {
        id: (Date.now() + 1).toString(),  // ← Add this line
        type: 'bot',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl h-[80vh] flex flex-col">
        ...
        {Array.isArray(chatMessages) && chatMessages.map((msg, index) => (
          <div key={msg.id || `message-${index}`} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`rounded-lg p-4 ${
                  ...
                  {msg.type === 'bot' && !msg.isError && (
                    <div className="mt-3 space-y-2">
                      ...
                      {/* Suggested actions */}
                      {msg.suggestedActions && Array.isArray(msg.suggestedActions) && msg.suggestedActions.length > 0 && (
